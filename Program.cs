...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            List<string> alpha = shows.OrderBy(s => s.Name).Select(s => s.Name).ToList();
            return alpha;
        }
        ...
        {
            //List<Show> vari = shows.OrderBy(s => s.ImdbRating).Take(1).ToList();
            return shows.Where(s => s.ImdbRating > shows.OrderBy(s => s.ImdbRating).Take(1).ToList().First().ImdbRating).ToList();
        ...
        {
            return shows.Where(s => s.EpisodeCount < 100).ToList().Select(s => s.Name).ToList();
        }
        ...
        {
            List<Show> shws = shows.Where(s => s.Genres.Contains("Comedy")).OrderByDescending(s => s.ImdbRating).ToList();
            return shows.SelectMany(s => s.Genres).Distinct().ToList();
            // return shws.Select(s => s.Name).ToList();
        }
        ...
        {
            return shows.OrderBy(s => s.StartYear).ToList().Where(s => s.Genres.Count > 1).ToList();
        }

        // 15. Return the show with the most episodes.
        static Show MostEpisodes(List<Show> shows)
        {
            return shows.OrderByDescending(s => s.EpisodeCount).ToList().First();
        }
        ...
        {
            return shows.OrderBy(s => s.EndYear).ToList().FirstOrDefault(s => s.EndYear > 1999);
        }
        ...
        {
            return shows.Where(s => s.Genres.Contains("Drama")).ToList().OrderByDescending(s => s.ImdbRating).ToList().Skip(1).ToList();
        }

        // 19. Return the number of crime shows with an IMDB rating greater than 7.0.
        static int GoodCrimeShows(List<Show> shows)
        {
            return shows.Where(s => s.ImdbRating > 7.0).ToList().Where(s => s.Genres.Contains("Crime")).ToList().Count();
        }
        ...
        {
            return shows.Where(s => (s.EndYear - s.StartYear) > 10).ToList().Where(s => s.ImdbRating < 8.0).ToList().OrderBy(s => s.Name).ToList().First();
        }

        // 21. Return the show with the most words in the name.
        static Show WordieastName(List<Show> shows)
        {
            return shows.OrderByDescending(s => s.Name.Count(n => n == ' ')).ToList().First();
        }
        ...
        **************************************************************************************************/
        ...
        static List<String> b1(List<Show> shows)
        {
            // 1. Return the genres of the shows that started in the 80s.

            return shows.Where(s => s.StartYear < 1990).ToList().SelectMany(s => s.Genres).ToList().Distinct().ToList();
        }

        // 2. Print a unique list of generes.
        static List<String> b2(List<Show> shows)
        {
            return shows.SelectMany(s => s.Genres).Distinct().ToList();
        }
